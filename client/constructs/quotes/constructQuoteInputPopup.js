...
import { create, update } from "./api-constructQuote.js";
import ConstructQuoteForm from "./constructQuoteForm";
import ConstructQuoteEditor from "./constructQuoteEditor";
...
export default function ConstructQuoteInputPopup(props) {
  const { formValues,
    setFormValues,
    setOpenPopup,
    setSelected,
...
const createNewConstructQuote = () => {
  console.log("ConstructQuoteInputPopup - createNewConstructQuote = ");
  const newDate = Date.now();
  console.log(
    "ConstructQuoteInputPopup - just before call to create quote formValues = ",
    formValues
  );
  let newConstruct =  {
      ...
      constructQuote: formValues.constructQuote ,
  ...
  create(
    ...
    if (!data) {
      console.log(
        "ConstructQuoteInputPopup - No Data so dispatch, Check Criteria"
      );
      setNotify({
        isOpen: true,
        message: "Quote not created! Please try again.. Notify Administrator if problem persists.",
        type: "error",
      });
    } else {
      if (data.error) {
        console.log(
          "ConstructQuoteInputPopup - error returned = " + data.error
        );
        setNotify({
          isOpen: true,
          message: "Quote not created! Please try again.. Notify Administrator if problem persists.",
        ...
        console.log("ConstructQuoteInputPopup - after create - data = ", data );
        console.log("ConstructQuoteInputPopup - after create - data = ", data._id );
        ...
        console.log("ConstructQuoteInputPopup - after create - formValues = ", formValues );
        setNotify({
          isOpen: true,
...
const updateExistingConstructQuote = (matchingIndex) => {
...
update(
    ...
    if (!data) {
      ...
      setNotify({
        isOpen: true,
        message: "Quote not update! Please try again.. Notify Administrator if problem persists.",
        type: "error",
      });
    } else {
      if (data.error) {
        ...
        console.log("ConstructQuoteInputPopup - after create - preparingFormValues with data = ", data );
        console.log("ConstructQuoteInputPopup - after create - preparingFormValues with data = ", data._id );
...
const SaveEntries = () => {
  ...
  console.log(
    "ConstructQuoteInputPopup - SaveEntries clicked before create/update formValues = ",
    formValues
  );
  if (
    formValues.topic.length == 0 ||
    formValues.description.length == 0 ||
     formValues.constructQuote.quote.length == 0
  ) {
    setFormValues({
      ...formValues,
      modified: true,
      isError: true,
      errorMessage:
        "Error alert!!! You must have an entry for topic, description and quote before saving!",
    });
    return;
  }
  if (
  ...
  console.log("ConstructQuoteInputPopup - search for id =  ", formValues._id); 
  const matchingIndex = findMatchingIndex(formValues._id);
  console.log(
    "ConstructQuoteInputPopup - after matchingIndex =  " + matchingIndex
  );
  if (matchingIndex >= 0) {
    updateExistingConstructQuote(matchingIndex);
  } else {
    createNewConstructQuote();
  }
  console.log("ConstructQuoteInputPopup End of Save Entries !!");
};
