...
import { listByCriteria, update, readById } from "./api-constructSuffix.js";
import ConstructSuffixImport from "./constructSuffixImport"
...
import { getCurrentConstructByMaxUpdateDateAndUserId } from "./api-constructSuffix";
...
export default function ConstructSuffixContainer() {
  ...
  const formTitle = "Suffix Entry and Update Form";
  const listingTitle = "Suffix Listing";
  ...
  const [
  constructSuffixsPreFilter,
  setConstructSuffixsPreFilter,
] = useState([]);
const [notify, setNotify] = useState({
  ...
  console.log(
    "constructSuffixContainer - right after initiation of formValues = ",
    formValues
  );

   console.log(
    "constructSuffixContainer - right after start of currentCriteria = ",
    currentCriteria
  );
  console.log(
    "constructSuffixContainer - right after start of currentAccess = ",
    currentAccess
  );
  ...
  {
    console.log(
      "constructSuffixContainer - checkCurrentCriteria = ",
      currentCriteria
    );
    if (valuesToCheck != undefined && valuesToCheck != null)
    {
      ...
      {
        console.log(
          "constructSuffixContainer - checkCurrentCriteria - Criteria found to have changed, valuesToCheck = ",
          valuesToCheck
        );
        criteriaDispatch({
        ...
        console.log(
          "constructSuffixContainer - checkCurrentCriteria - Criteria did not change" );
      }
    }      else{
      console.log(
        "constructSuffixContainer - checkCurrentCriteria - Criteria did not change" );
...
{
  console.log(
    "constructSuffixContainer - checkCurrentAccess = ",
    currentAccess
  );
  if (valuesToCheck != undefined && valuesToCheck != null)
  {
    if (valuesToCheck.keepPrivate != currentAccess.keepPrivate ||
      ...
      {
        console.log(
          "constructSuffixContainer - checkCurrentAccess - Access found to have changed, valuesToCheck = ",
          valuesToCheck
        );
      accessDispatch({
      ...
      console.log(
        "constructSuffixContainer - checkCurrentAccess - Access did not change" );
    }
  }  else{
    console.log(
      "constructSuffixContainer - checkCurrentAccess - Access did not change" );
  ...
  const markItemDeleted = (indexToDelete, markDeleted)=>{
    setConstructSuffixs(
    [
      ...constructSuffixs.slice(0, indexToDelete), // everything before current post
      {
        ...constructSuffixs[indexToDelete],
        markDeleted: markDeleted,
      },
      ...constructSuffixs.slice(indexToDelete + 1), // everything after current post
    ]);
    setConstructSuffixsPreFilter(
      [
        ...constructSuffixs.slice(0, indexToDelete), // everything before current post
  ...
  const updateConstructLineItem = (indexToUpdate, toggleDelete, lastIndexToUpdate, newCriteria, newAccess) => {
    console.log(
      "constructSuffixContainer - updateConstructLineItem Suffix #indexToUpdate = ", indexToUpdate );
    const markDeleted = toggleDelete? (!constructSuffixs[indexToUpdate].markDeleted):constructSuffixs[indexToUpdate].markDeleted;
    console.log("constructSuffixContainer - updateConstructLineItem constructSuffixs[indexToUpdate] = ", constructSuffixs[indexToUpdate] );
    let recordToUpdate = {};
    const id = constructSuffixs[indexToUpdate]._id
    ...
    readById(
      ...
      if (!data) {
        console.log(
          "constructSuffix Container - updateConstructLineItem -readById - No Data Found!!!!"
        );
        recordToUpdate = null;
      } else {
        if (data.error) {
          console.log(
            "constructSuffix Container - updateConstructLineItem - Error!!!!", data.error
          );
          recordToUpdate = null;
        } else {
          console.log(
            "constructSuffix Container - updateConstructLineItem =  ",
            data
          );
          recordToUpdate = data;
          recordToUpdate.markDeleted = markDeleted
          updateConstructSuffix (recordToUpdate,indexToUpdate, lastIndexToUpdate, toggleDelete, newCriteria, newAccess)
        }
      }
    });    
  }

const updateConstructSuffix = (recordToUpdate, indexToUpdate, lastIndexToUpdate, toggleDelete, newCriteria, newAccess) =>
{
    ...
    update(
      ...
      if (!data) {
        console.log(
          "constructSuffixContainer - updateConstructSuffix - No Data so dispatch, Check Criteria"
        );
        setNotify({
          isOpen: true,
          message: "Suffix update failed! Please try again.. Notify Administrator if problem persists.",
          type: "error",
        });
      } else {
        if (data.error) {
          console.log(
            "constructSuffixContainer - updateConstructSuffix - error returned = " + data.error
          );
          setNotify({
            isOpen: true,
            message: "Suffix update failed! Please try again.. Notify Administrator if problem persists.",
          ...
          console.log("constructSuffixContainer -updateConstructSuffix -  Update Successful!!! data =  " + data); 
          if (toggleDelete)
          {
            console.log("constructSuffixContainer - updates notComplete current index = ", indexToUpdate  )
            markItemDeleted(indexToUpdate,recordToUpdate.markDeleted)          
            setNotify({
              isOpen: true,
              message: "Suffix Toggle Deleted/Undeleted",
              type: "success",
            });
          }
          if (indexToUpdate == lastIndexToUpdate)
          {
            ...
            setNotify({
              isOpen: true,
              message: "Suffix record(s) updated!",
              type: "success",
            });
          }
        }
 ...
 const prepareFormValues = (newFormValues) =>
  {
    console.log("ConstructSuffixConatiner - preparingFormValues with newFormValues = ", newFormValues );
    setFormValues({
      ...
      subType: (newFormValues != null && newFormValues.subType)  ? newFormValues.subType : "Suffix",
      ...
      constructSuffix: (newFormValues != null && newFormValues.constructSuffix)  ? newFormValues.constructSuffix : {
        suffix: "",	
        meaning: "",	
        etymology: "",	
        type: "",	
  ...
  useEffect(() => {
    ...
    console.log (" constructSuffixContainer - useEffect - to populate the groupsUserOwns = ",userData.groupsUserOwns)
  let groupOptions = []  
  ...
  useEffect(() => {
    const abortController = new AbortController();
    const signal = abortController.signal;
    console.log("ConstructSuffixContainer - inside useEffect()  ");
    getCurrentConstructByMaxUpdateDateAndUserId(
      ...
      if (!data) {
        console.log(
          "ConstructSuffixContainer - No Data so dispatch, Check Criteria"
        );
      } else {
        if (data.error) {
          console.log(
            "ConstructSuffixContainer - No Data so error is returned data =: ",
            data
          );
          console.log(
            "ConstructSuffixContainer - No Data so error is returned data.error =: ",
            data.error
          );
          setRedirectToSignin(true);
        } else {
          console.log(
            "ConstructSuffixContainer - Right after data is returned data =: ",
            data
          );
          criteriaDispatch({
            type: "SET_CRITERIAFROMCURRENTITEM",
  ...
  useEffect(() => {
    ...
    const signal = abortController.signal;
      console.log(
        "constructSuffixContainer - Update Listing whenever the critiera changes, currentCriteria = ",
        currentCriteria
      );
          ...
          listByCriteria(
            ...
            if (!data) {
              ...
              if (data.error) {
                setLoading(false);
                console.log(
                  "constructSuffixContainer - error returned = " + data.error
                );
                setConstructSuffixs([]);
                setConstructSuffixsPreFilter([]);
                ...
                setLoading(false);
                console.log(
                  "constructSuffixContainer - Update listing by ListByCriteria =  ",
                  data
                );
                const resultArray = data.map(suffix => ({  topic: suffix.topic,
                                                                description: suffix.description,
                                                                owner_id: suffix.owner_id,
                                                                group_id: suffix.group_id,
                                                                constructDetail: suffix.description,
                                                                markDeleted: suffix.markDeleted,
                                                                _id:suffix._id,}));
                setConstructSuffixs(resultArray);
                setConstructSuffixsPreFilter(resultArray);
  ...
  const handleRequestImport = () => {
    console.log ("constructSuffixContainer - setting popup for Import")
  ...
  return (
      ...
      <CssBaseline />
          <Grid container spacing={0} justify="center" className={classes.container} >         
            <Grid item xs={9} >             
                ...
                constructLineItems = {constructSuffixs}
                setConstructLineItems = {setConstructSuffixs}
                constructLineItemsPreFilter ={constructSuffixsPreFilter}
                setConstructLineItemsPreFilter ={setConstructSuffixsPreFilter}
    ...
    <Popup
          title="Import Multiple Suffixs from a file"
          openPopup={openImportPopup}
          setOpenPopup={setOpenImportPopup}
      >
          <ConstructSuffixImport
