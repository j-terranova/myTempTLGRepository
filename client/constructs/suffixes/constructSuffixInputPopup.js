...
import { create, update } from "./api-constructSuffix.js";
import ConstructSuffixForm from "./constructSuffixForm";
import ConstructSuffixEditor from "./constructSuffixEditor";
...
export default function ConstructSuffixInputPopup(props) {
  const { formValues,
    setFormValues,
    setOpenPopup,
    setSelected,
  ...
  setConstructLineItems(
    [
      ...constructLineItems,
      {
        ...
        constructDetail: newConstructLineItem.constructSuffix.suffix,
  ...
  setConstructLineItems(
    [
      ...constructLineItems.slice(0, index), // everything before current post
      {
        ...
        constructDetail: updatedConstructLineItem.constructSuffix.suffix,
...
const createNewConstructSuffix = () => {
  console.log("ConstructSuffixInputPopup - createNewConstructSuffix = ");
  const newDate = Date.now();
  console.log(
    "ConstructSuffixInputPopup - just before call to create suffix formValues = ",
    formValues
  );
  let newConstruct =  {
      ...
      constructSuffix: formValues.constructSuffix ,
  ...
  create(
    ...
    if (!data) {
      console.log(
        "ConstructSuffixInputPopup - No Data so dispatch, Check Criteria"
      );
      setNotify({
        isOpen: true,
        message: "Suffix not created! Please try again.. Notify Administrator if problem persists.",
        type: "error",
      });
    } else {
      if (data.error) {
        console.log(
          "ConstructSuffixInputPopup - error returned = " + data.error
        );
        setNotify({
          isOpen: true,
          message: "Suffix not created! Please try again.. Notify Administrator if problem persists.",
        ...
        console.log("ConstructSuffixInputPopup - after create - data = ", data );
        console.log("ConstructSuffixInputPopup - after create - data = ", data._id );
        ...
        console.log("ConstructSuffixInputPopup - after create - formValues = ", formValues );
        setNotify({
          isOpen: true,
          message: "Suffix Created",
          type: "success",
        });
      }
    }
...
const updateExistingConstructSuffix = (matchingIndex) => {
...
update(
    ...
    if (!data) {
      ...
      setNotify({
        isOpen: true,
        message: "Suffix not update! Please try again.. Notify Administrator if problem persists.",
        type: "error",
      });
    } else {
      if (data.error) {
        ...
        console.log("ConstructSuffixInputPopup - after create - preparingFormValues with data = ", data );
        console.log("ConstructSuffixInputPopup - after create - preparingFormValues with data = ", data._id );
...
const SaveEntries = () => {
  ...
  console.log(
    "ConstructSuffixInputPopup - SaveEntries clicked before create/update formValues = ",
    formValues
  );
  if (
    formValues.topic.length == 0 ||
    formValues.description.length == 0 ||
    formValues.constructSuffix.suffix.length == 0
  ) {
    setFormValues({
      ...formValues,
      modified: true,
      isError: true,
      errorMessage:
        "Error alert!!! You must have an entry for topic, description and suffix before saving!",
    });
    return;
  }
  if (
  ...
  console.log("ConstructSuffixInputPopup - search for id =  ", formValues._id); 
  const matchingIndex = findMatchingIndex(formValues._id);
  console.log(
    "ConstructSuffixInputPopup - after matchingIndex =  " + matchingIndex
  );
  if (matchingIndex >= 0) {
    updateExistingConstructSuffix(matchingIndex);
  } else {
    createNewConstructSuffix();
  }
  console.log("ConstructSuffixInputPopup End of Save Entries !!");
};
