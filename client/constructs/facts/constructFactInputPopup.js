...
import { create, update } from "./api-constructFact.js";
import ConstructFactForm from "./constructFactForm";
import ConstructFactEditor from "./constructFactEditor";
...
export default function ConstructFactInputPopup(props) {
  const { formValues,
    setFormValues,
    setOpenPopup,
    setSelected,
...
const createNewConstructFact = () => {
  console.log("ConstructFactInputPopup - createNewConstructFact = ");
  const newDate = Date.now();
  console.log(
    "ConstructFactInputPopup - just before call to create fact formValues = ",
    formValues
  );
  let newConstruct =  {
      ...
      constructFact: formValues.constructFact ,
  ...
  create(
    ...
    if (!data) {
      console.log(
        "ConstructFactInputPopup - No Data so dispatch, Check Criteria"
      );
      setNotify({
        isOpen: true,
        message: "Fact not created! Please try again.. Notify Administrator if problem persists.",
        type: "error",
      });
    } else {
      if (data.error) {
        console.log(
          "ConstructFactInputPopup - error returned = " + data.error
        );
        setNotify({
          isOpen: true,
          message: "Fact not created! Please try again.. Notify Administrator if problem persists.",
        ...
        console.log("ConstructFactInputPopup - after create - data = ", data );
        console.log("ConstructFactInputPopup - after create - data = ", data._id );
        ...
        console.log("ConstructFactInputPopup - after create - formValues = ", formValues );
        setNotify({
          isOpen: true,
...
const updateExistingConstructFact = (matchingIndex) => {
...
update(
    ...
    if (!data) {
      ...
      setNotify({
        isOpen: true,
        message: "Fact not update! Please try again.. Notify Administrator if problem persists.",
        type: "error",
      });
    } else {
      if (data.error) {
        ...
        console.log("ConstructFactInputPopup - after create - preparingFormValues with data = ", data );
        console.log("ConstructFactInputPopup - after create - preparingFormValues with data = ", data._id );
...
const SaveEntries = () => {
  ...
  console.log(
    "ConstructFactInputPopup - SaveEntries clicked before create/update formValues = ",
    formValues
  );
  if (
    formValues.topic.length == 0 ||
    formValues.description.length == 0 ||
     formValues.constructFact.fact.length == 0
  ) {
    setFormValues({
      ...formValues,
      modified: true,
      isError: true,
      errorMessage:
        "Error alert!!! You must have an entry for topic, description and fact before saving!",
    });
    return;
  }
  if (
  ...
  console.log("ConstructFactInputPopup - search for id =  ", formValues._id); 
  const matchingIndex = findMatchingIndex(formValues._id);
  console.log(
    "ConstructFactInputPopup - after matchingIndex =  " + matchingIndex
  );
  if (matchingIndex >= 0) {
    updateExistingConstructFact(matchingIndex);
  } else {
    createNewConstructFact();
  }
  console.log("ConstructFactInputPopup End of Save Entries !!");
};
