...
import { listByCriteria, update, readById } from "./api-constructFact.js";
...
import { getCurrentConstructByMaxUpdateDateAndUserId } from "./api-constructFact";
...
export default function ConstructFactContainer() {
  ...
  const formTitle = "Fact Entry and Update Form";
  const listingTitle = "Fact Listing";
  ...
  const [
   ...
   const [
    constructFactsPreFilter,
    setConstructFactsPreFilter,
  ] = useState([]);
  ...
  console.log(
    "constructFactContainer - right after initiation of formValues = ",
    formValues
  );

  console.log(
    "constructFactContainer - right after start of currentCriteria = ",
    currentCriteria
  );
  console.log(
    "constructFactContainer - right after start of currentAccess = ",
    currentAccess
  );
  ...
  {
    console.log(
      "constructFactContainer - checkCurrentCriteria = ",
      currentCriteria
    );
    if (valuesToCheck != undefined && valuesToCheck != null)
    {
      ...
      {
        console.log(
          "constructFactContainer - checkCurrentCriteria - Criteria found to have changed, valuesToCheck = ",
          valuesToCheck
        );
        criteriaDispatch({
        ...
        console.log(
          "constructFactContainer - checkCurrentCriteria - Criteria did not change" );
      }
    }      else{
      console.log(
        "constructFactContainer - checkCurrentCriteria - Criteria did not change" );
...
{
  console.log(
    "constructFactContainer - checkCurrentAccess = ",
    currentAccess
  );
  if (valuesToCheck != undefined && valuesToCheck != null)
  {
    if (valuesToCheck.keepPrivate != currentAccess.keepPrivate ||
      ...
      {
        console.log(
          "constructFactContainer - checkCurrentAccess - Access found to have changed, valuesToCheck = ",
          valuesToCheck
        );
      accessDispatch({
      ...
      console.log(
        "constructFactContainer - checkCurrentAccess - Access did not change" );
    }
  }  else{
    console.log(
      "constructFactContainer - checkCurrentAccess - Access did not change" );
  ...
  const markItemDeleted = (indexToDelete, markDeleted)=>{
    setConstructFacts(
    [
      ...constructFacts.slice(0, indexToDelete), // everything before current post
      {
        ...constructFacts[indexToDelete],
        markDeleted: markDeleted,
      },
      ...constructFacts.slice(indexToDelete + 1), // everything after current post
    ]);
    setConstructFactsPreFilter(
      [
        ...constructFacts.slice(0, indexToDelete), // everything before current post
  ...
  const updateConstructLineItem = (indexToUpdate, toggleDelete, lastIndexToUpdate, newCriteria, newAccess) => {
    console.log(
      "constructFactContainer - updateConstructLineItem Fact #indexToUpdate = ", indexToUpdate );
    const markDeleted = toggleDelete? (!constructFacts[indexToUpdate].markDeleted):constructFacts[indexToUpdate].markDeleted;
    console.log("constructFactContainer - updateConstructLineItem constructFacts[indexToUpdate] = ", constructFacts[indexToUpdate] );
    let recordToUpdate = {};
    const id = constructFacts[indexToUpdate]._id
    ...
    readById(
      ...
      if (!data) {
        console.log(
          "constructFact Container - updateConstructLineItem -readById - No Data Found!!!!"
        );
        recordToUpdate = null;
      } else {
        if (data.error) {
          console.log(
            "constructFact Container - updateConstructLineItem - Error!!!!", data.error
          );
          recordToUpdate = null;
        } else {
          console.log(
            "constructFact Container - updateConstructLineItem =  ",
            data
          );
          recordToUpdate = data;
          recordToUpdate.markDeleted = markDeleted
          updateConstructFact (recordToUpdate,indexToUpdate, lastIndexToUpdate, toggleDelete, newCriteria, newAccess)
        }
      }
    });    
  }

const updateConstructFact = (recordToUpdate, indexToUpdate, lastIndexToUpdate, toggleDelete, newCriteria, newAccess) =>
{
    ...
    update(
      ...
      if (!data) {
        console.log(
          "constructFactContainer - updateConstructFact - No Data so dispatch, Check Criteria"
        );
        setNotify({
          isOpen: true,
          message: "Fact update failed! Please try again.. Notify Administrator if problem persists.",
          type: "error",
        });
      } else {
        if (data.error) {
          console.log(
            "constructFactContainer - updateConstructFact - error returned = " + data.error
          );
          setNotify({
            isOpen: true,
            message: "Fact update failed! Please try again.. Notify Administrator if problem persists.",
          ...
          console.log("constructFactContainer -updateConstructFact -  Update Successful!!! data =  " + data); 
          if (toggleDelete)
          {
            console.log("constructFactContainer - updates notComplete current index = ", indexToUpdate  )
            markItemDeleted(indexToUpdate,recordToUpdate.markDeleted)          
            setNotify({
              isOpen: true,
              message: "Fact Toggle Deleted/Undeleted",
              type: "error",
            });
          }
 ...
 const prepareFormValues = (newFormValues) =>
  {
    console.log("ConstructFactConatiner - preparingFormValues with newFormValues = ", newFormValues );
    setFormValues({
      ...
      subType: (newFormValues != null && newFormValues.subType)  ? newFormValues.subType : "Fact",
      ...
      constructFact: (newFormValues != null && newFormValues.constructFact)  ? newFormValues.constructFact : {
              fact:"",
              source: "",	
              type: "",
              proof: [],
  ...
  useEffect(() => {
    ...
    console.log (" constructFactContainer - useEffect - to populate the groupsUserOwns = ",userData.groupsUserOwns)
  let groupOptions = []  
  ...
  useEffect(() => {
    const abortController = new AbortController();
    const signal = abortController.signal;
    console.log("ConstructFactContainer - inside useEffect()  ");
    getCurrentConstructByMaxUpdateDateAndUserId(
      ...
      if (!data) {
        console.log(
          "ConstructFactContainer - No Data so dispatch, Check Criteria"
        );
      } else {
        if (data.error) {
          console.log(
            "ConstructFactContainer - No Data so error is returned data =: ",
            data
          );
          console.log(
            "ConstructFactContainer - No Data so error is returned data.error =: ",
            data.error
          );
          setRedirectToSignin(true);
        } else {
          console.log(
            "ConstructFactContainer - Right after data is returned data =: ",
            data
          );
          criteriaDispatch({
            type: "SET_CRITERIAFROMCURRENTITEM",
  ...
  useEffect(() => {
    ...
    const signal = abortController.signal;
      console.log(
        "constructFactContainer - Update Listing whenever the critiera changes, currentCriteria = ",
        currentCriteria
      );
      if (currentCriteria != undefined && currentCriteria != null )
      {
        if (
          ...
          listByCriteria(
            ...
            if (!data) {
              console.log(
                "ConstructFactImport - No Data so dispatch, Check Criteria"
              );
              setConstructFacts([]);
              setConstructFactsPreFilter([]);
            } else {
              if (data.error) {
                console.log(
                  "constructFactContainer - error returned = " + data.error
                );
                setConstructFacts([]);
                setConstructFactsPreFilter([]);
              } else if (data.length > 0) {
                console.log(
                  "constructFactContainer - Update listing by ListByCriteria =  ",
                  data
                );
                const resultArray = data.map(fact => ({  topic: fact.topic,
                                                              description: fact.description,
                                                              owner_id: fact.owner_id,
                                                              group_id: fact.group_id,
                                                              constructDetail: fact.constructFact.fact,
                                                              markDeleted: fact.markDeleted,
                                                              _id:fact._id,}));

                setConstructFacts(resultArray);
                setConstructFactsPreFilter(resultArray);
      ...
      <CssBaseline />
          <Grid container spacing={0} justify="center" className={classes.container} >         
            <Grid item xs={9} >             
                ...
                constructLineItems = {constructFacts}
                setConstructLineItems = {setConstructFacts}
                constructLineItemsPreFilter ={constructFactsPreFilter}
                setConstructLineItemsPreFilter ={setConstructFactsPreFilter}
