...
import { loadGutenbergAssociationData } from "./api-constructAssociation.js";
...
export default function ConstructAssociationImport(props) {
...
console.log("ConstructAssociationImport" );
...
console.log("ConstructAssociationImport - after setImportValues = ",importValues );
...
const handleImportFileChange = e => {
  ...
  const handleImportAssociations = e => {
      e.preventDefault()
      if (validate()) {
        loadFile(); 
      }
  ...
  const loadFile = () => {
    loadGutenbergAssociationData(
          { userId: jwt.user._id, 
            skeletonAssociation: importValues,
          },
          {
            t: jwt.token,
          }
      ...
      if (!data) {
        ...
        if (data.error) {
          console.log(
            "ConstructAssociationImport - load error returned = " + data.error
          );
        } else {
          console.log("ConstructAssociationImport - load data =  " + data);

          }
          setNotify({
            isOpen: true,
            message: "File Load of Associations Completed",
            type: "success",
          });
        }
      }
        ...
        <Form onSubmit={handleSubmit}>
            ...
            <Grid item xs={12}>
              <Typography variant="h6" className={classes.title}>
                Each new association record will include entries as you have assigned them on this form.
                You can enter or leave blank entries for topic, tittle and/or description.
                Imput file must be comma delimited.  
                Input file must contain the association.  The file may also contain; topic, title and description.
                ...
                Association must always be included.  It can be the only file entry if other data is on the form.
              </Typography>
            </Grid>

            <Grid item xs={6}>
                   <Controls.Button
                    text="Import Associations from Association File"
                    variant="contained"
                    color = "primary"
                    endIcon={<ImportExportIcon/>}
                    onClick={handleImportAssociations}
                />
            </Grid> 
            <Grid item xs={6}>
                <Controls.Button
            ...
            <Grid item xs={6}>
                ...
                <Grid item xs={6}>
                    <div>
                        ...
                        Access to be applied to imported associations
                ...
                Criteria/Tags to be applied to imported associations
