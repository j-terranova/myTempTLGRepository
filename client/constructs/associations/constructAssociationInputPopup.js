...
import { create, update } from "./api-constructAssociation.js";
import ConstructAssociationForm from "./constructAssociationForm";
import ConstructAssociationEditor from "./constructAssociationEditor";
...
export default function ConstructAssociationInputPopup(props) {
  const { formValues,
    setFormValues,
    setOpenPopup,
    setSelected,
  ...
  setConstructLineItems(
    [
      ...constructLineItems,
      {
        ...
        constructDetail: newConstructLineItem.constructAssociation.wordToAssociate,
  ...
  setConstructLineItems(
    [
      ...constructLineItems.slice(0, index), // everything before current post
      {
        ...
        constructDetail: updatedConstructLineItem.constructAssociation.wordToAssociate,
...
const createNewConstructAssociation = () => {
  console.log("ConstructAssociationInputPopup - createNewConstructAssociation = ");
  const newDate = Date.now();
  console.log(
    "ConstructAssociationInputPopup - just before call to create association formValues = ",
    formValues
  );
  let newConstruct =  {
      ...
      constructAssociation: formValues.constructAssociation ,
  ...
  create(
    ...
    if (!data) {
      console.log(
        "ConstructAssociationInputPopup - No Data so dispatch, Check Criteria"
      );
      setNotify({
        isOpen: true,
      ...
      if (data.error) {
        console.log(
          "ConstructAssociationInputPopup - error returned = " + data.error
        );
        setNotify({
          isOpen: true,
          message: "Association not created! Please try again.. Notify Administrator if problem persists.",
        ...
        console.log("ConstructAssociationInputPopup - after create - data = ", data );
        console.log("ConstructAssociationInputPopup - after create - data = ", data._id );
        ...
        console.log("ConstructAssociationInputPopup - after create - formValues = ", formValues );
        setNotify({
          isOpen: true,
...
const updateExistingConstructAssociation = (matchingIndex) => {
...
update(
    ...
    if (!data) {
      ...
      if (data.error) {
        ...
        console.log("ConstructAssociationInputPopup - after create - preparingFormValues with data = ", data );
        console.log("ConstructAssociationInputPopup - after create - preparingFormValues with data = ", data._id );
...
const SaveEntries = () => {
  ...
  console.log(
    "ConstructAssociationInputPopup - SaveEntries clicked before create/update formValues = ",
    formValues
  );
  if (
    formValues.topic.length == 0 ||
    formValues.description.length == 0 ||
    formValues.constructAssociation.wordToAssociate.length == 0
  ) {
    setFormValues({
      ...formValues,
      modified: true,
  ...
  console.log("ConstructAssociationInputPopup - search for id =  ", formValues._id); 
  const matchingIndex = findMatchingIndex(formValues._id);
  console.log(
    "ConstructAssociationInputPopup - after matchingIndex =  " + matchingIndex
  );
  if (matchingIndex >= 0) {
    updateExistingConstructAssociation(matchingIndex);
  } else {
    createNewConstructAssociation();
  }
  console.log("ConstructAssociationInputPopup End of Save Entries !!");
};
