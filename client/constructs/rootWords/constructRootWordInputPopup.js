...
import { create, update } from "./api-constructRootWord.js";
import ConstructRootWordForm from "./constructRootWordForm";
import ConstructRootWordEditor from "./constructRootWordEditor";
...
export default function ConstructRootWordInputPopup(props) {
  const { formValues,
    setFormValues,
    setOpenPopup,
    setSelected,
  ...
  setConstructLineItems(
    [
      ...constructLineItems,
      {
        ...
        constructDetail: newConstructLineItem.constructRootWord.rootWord,
  ...
  setConstructLineItems(
    [
      ...constructLineItems.slice(0, index), // everything before current post
      {
        ...
        constructDetail: updatedConstructLineItem.constructRootWord.rootWord,
...
const createNewConstructRootWord = () => {
  console.log("ConstructRootWordInputPopup - createNewConstructRootWord = ");
  const newDate = Date.now();
  console.log(
    "ConstructRootWordInputPopup - just before call to create rootWord formValues = ",
    formValues
  );
  let newConstruct =  {
      ...
      constructRootWord: formValues.constructRootWord ,
  ...
  create(
    ...
    if (!data) {
      console.log(
        "ConstructRootWordInputPopup - No Data so dispatch, Check Criteria"
      );
      setNotify({
        isOpen: true,
        message: "RootWord not created! Please try again.. Notify Administrator if problem persists.",
        type: "error",
      });
    } else {
      if (data.error) {
        console.log(
          "ConstructRootWordInputPopup - error returned = " + data.error
        );
        setNotify({
          isOpen: true,
          message: "RootWord not created! Please try again.. Notify Administrator if problem persists.",
        ...
        console.log("ConstructRootWordInputPopup - after create - data = ", data );
        console.log("ConstructRootWordInputPopup - after create - data = ", data._id );
        ...
        console.log("ConstructRootWordInputPopup - after create - formValues = ", formValues );
        setNotify({
          isOpen: true,
          message: "RootWord Created",
          type: "success",
        });
      }
    }
...
const updateExistingConstructRootWord = (matchingIndex) => {
...
update(
    ...
    if (!data) {
      ...
      setNotify({
        isOpen: true,
        message: "RootWord not update! Please try again.. Notify Administrator if problem persists.",
        type: "error",
      });
    } else {
      if (data.error) {
        ...
        console.log("ConstructRootWordInputPopup - after create - preparingFormValues with data = ", data );
        console.log("ConstructRootWordInputPopup - after create - preparingFormValues with data = ", data._id );
...
const SaveEntries = () => {
  ...
  console.log(
    "ConstructRootWordInputPopup - SaveEntries clicked before create/update formValues = ",
    formValues
  );
  if (
    formValues.topic.length == 0 ||
    formValues.description.length == 0 ||
    formValues.constructRootWord.rootWord.length == 0
  ) {
    setFormValues({
      ...formValues,
      modified: true,
      isError: true,
      errorMessage:
        "Error alert!!! You must have an entry for topic, description and rootWord before saving!",
    });
    return;
  }
  if (
  ...
  console.log("ConstructRootWordInputPopup - search for id =  ", formValues._id); 
  const matchingIndex = findMatchingIndex(formValues._id);
  console.log(
    "ConstructRootWordInputPopup - after matchingIndex =  " + matchingIndex
  );
  if (matchingIndex >= 0) {
    updateExistingConstructRootWord(matchingIndex);
  } else {
    createNewConstructRootWord();
  }
  console.log("ConstructRootWordInputPopup End of Save Entries !!");
};
