...
import { loadGutenbergRootWordData } from "./api-constructRootWord.js";
...
export default function ConstructRootWordImport(props) {
...
console.log("ConstructRootWordImport" );
...
console.log("ConstructRootWordImport - after setImportValues = ",importValues );
  ...
  const loadFile = () => {
    loadGutenbergRootWordData(
      ...
      if (!data) {
        ...
        if (data.error) {
          console.log(
            "ConstructRootWordImport - load error returned = " + data.error
          );
        } else {
          console.log("ConstructRootWordImport - load data =  " + data);

          }
          setNotify({
            isOpen: true,
            message: "File Load of RootWords Completed",
            type: "success",
          });
        }
      }
        ...
        <Form onSubmit={handleSubmit}>
            ...
            <Grid item xs={12}>
              <Typography variant="h6" className={classes.title}>
                Each new rootWord record will include entries as you have assigned them on this form.
                You can enter or leave blank entries for topic, tittle and/or description.
                Imput file must be comma delimited.  
                Input file must contain the rootWord.  The file may also contain; topic, title and description.
                ...
                RootWord must always be included.  It can be the only file entry if other data is on the form.
              </Typography>
            </Grid>

            <Grid item xs={6}>
                   <Controls.Button
                    text="Import RootWords from RootWord File"
                    variant="contained"
                    color = "primary"
                    endIcon={<ImportExportIcon/>}
                    onClick={handleImportRootWords}
                />
            </Grid> 
            <Grid item xs={6}>
                <Controls.Button
            ...
            <Grid item xs={6}>
                ...
                <Grid item xs={6}>
                    <div>
                        ...
                        Access to be applied to imported rootWords
                ...
                Criteria/Tags to be applied to imported rootWords
