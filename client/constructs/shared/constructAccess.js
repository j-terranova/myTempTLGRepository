...
export default function ConstructAccess(props) {
  const {
    selected,
    setSelected,
  ...
  console.log("constructAccess - right after values set ");
  ...
  useEffect(() => {
    ...
    console.log(
      "ConstructAccess - inside useEffect()after currentAccess Set  "
    );
  ...
  useEffect(() => {
    ...
    const signal = abortController.signal;
    console.log(
      "constructAccess - after return from popup useEffect applyOptions.isUpdateListing = ", applyOptions.isUpdateListing );
    console.log(
      "constructAccess - after return from popup useEffect applyOptions.isUpdateSelectedRecords = ", applyOptions.isUpdateSelectedRecords );
    console.log(
      "constructAccess - after return from popup useEffect applyOptions.isUpdateAllListedRecords = ", applyOptions.isUpdateAllListedRecords );

    if (applyOptions.isUpdateAllListedRecords === true)
    {
      console.log("constructAccess - applyOptions.isUpdateAllListedRecords === true" );
      updateAccessOfAllConstructLineItemsInList();
    } else if (applyOptions.isUpdateSelectedRecords === true)
    {
      console.log("constructAccess -applyOptions.isUpdateSelectedRecords === true" );
       updateAccessOfSelectedConstructLineItems();
    }
  ...
  const findMatchingIndex = (search_id) => {
    var index = -1;
    console.log("constructAccess - search_id = ", search_id);
    for (var i = 0; i < constructLineItems.length; i++) {
      console.log(
        "constructAccess - constructLineItems[i]._id = ",
  ...
  const updateAccessOfAllConstructLineItemsInList = () => {
      console.log(
        "updateAccessOfCurrentStatement - Inside updateAccessOfAllConstructLineItemsInList + before loop to update records");
      const lastIndexToUpdate = constructLineItems.length -1;
      for (let i = 0; i < constructLineItems.length; i++) {
        updateConstructLineItem(i, false, lastIndexToUpdate, currentCriteria, values);
      }
    };
  
    const updateAccessOfSelectedConstructLineItems = () =>
    {
      console.log(
        "updateAccessOfSelectedConstructLineItems - Inside updateAccessOfSelectedConstructLineItems + before loop to update records");
        ...
        selected.forEach(id => 
          {
            ...
            if (indexToUpdate >=0)
            {
              updateConstructLineItem(indexToUpdate, false, lastIndexToUpdate, currentCriteria, values);
            }            
          });
          setSelected([]);
    }
    ...
    const clickSubmit = () => {
      console.log(
        "constructAccess - submit clicked values before values = ",
        values
      );
       setOpenPopup(true);
     };
