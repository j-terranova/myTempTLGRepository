...
import { create, update } from "./api-constructMiniSegment.js";
import ConstructMiniSegmentForm from "./constructMiniSegmentForm";
...
export default function ConstructMiniSegmentInputPopup(props) {
  const { formValues,
    setFormValues,
    setOpenPopup,
    setSelected,
...
const createNewConstructMiniSegment = () => {
  console.log("ConstructMiniSegmentInputPopup - createNewConstructMiniSegment = ");
  const newDate = Date.now();
  console.log(
    "ConstructMiniSegmentInputPopup - just before call to create miniSegment formValues = ",
    formValues
  );
  let newConstruct =  {
      ...
      constructMiniSegment: formValues.constructMiniSegment ,
  ...
  create(
    ...
    if (!data) {
      console.log(
        "ConstructMiniSegmentInputPopup - No Data so dispatch, Check Criteria"
      );
      setNotify({
        isOpen: true,
        message: "MiniSegment not created! Please try again.. Notify Administrator if problem persists.",
        type: "error",
      });
    } else {
      if (data.error) {
        console.log(
          "ConstructMiniSegmentInputPopup - error returned = " + data.error
        );
        setNotify({
          isOpen: true,
          message: "MiniSegment not created! Please try again.. Notify Administrator if problem persists.",
        ...
        console.log("ConstructMiniSegmentInputPopup - after create - data = ", data );
        console.log("ConstructMiniSegmentInputPopup - after create - data = ", data._id );
        ...
        console.log("ConstructMiniSegmentInputPopup - after create - formValues = ", formValues );
        setNotify({
          isOpen: true,
          message: "MiniSegment Created",
          type: "success",
        });
      }
    }
...
const updateExistingConstructMiniSegment = (matchingIndex) => {
...
update(
    ...
    if (!data) {
      ...
      setNotify({
        isOpen: true,
        message: "MiniSegment not update! Please try again.. Notify Administrator if problem persists.",
        type: "error",
      });
    } else {
      if (data.error) {
        ...
        console.log("ConstructMiniSegmentInputPopup - after create - preparingFormValues with data = ", data );
        console.log("ConstructMiniSegmentInputPopup - after create - preparingFormValues with data = ", data._id );
...
const SaveEntries = () => {
  ...
  console.log(
    "ConstructMiniSegmentInputPopup - SaveEntries clicked before create/update formValues = ",
    formValues
  );
  if (
    formValues.topic.length == 0 ||
    formValues.description.length == 0 ||
    formValues.constructMiniSegment.length == 0
  ) {
    setFormValues({
      ...formValues,
      modified: true,
      isError: true,
      errorMessage:
        "Error alert!!! You must have an entry for topic, description and miniSegment before saving!",
    });
    return;
  }
  if (
  ...
  console.log("ConstructMiniSegmentInputPopup - search for id =  ", formValues._id); 
  const matchingIndex = findMatchingIndex(formValues._id);
  console.log(
    "ConstructMiniSegmentInputPopup - after matchingIndex =  " + matchingIndex
  );
  if (matchingIndex >= 0) {
    updateExistingConstructMiniSegment(matchingIndex);
  } else {
    createNewConstructMiniSegment();
  }
  console.log("ConstructMiniSegmentInputPopup End of Save Entries !!");
};
...
const validate = (fieldValues = formValues) => {
    ...
    if ('constructMiniSegment' in fieldValues)
        temp.constructMiniSegment = fieldValues.constructMiniSegment ? "" : "This field is required."
