...
import { listByCriteria, update, readById } from "./api-constructMiniSegment.js";
...
import { getCurrentConstructByMaxUpdateDateAndUserId } from "./api-constructMiniSegment";
...
export default function ConstructMiniSegmentContainer() {
  ...
  const formTitle = "MiniSegment Entry and Update Form";
  const listingTitle = "MiniSegment Listing";
  const [listAllWithoutCriteria, setListAllWithoutCriteria] = useState(false);
  const [groupsUserOwns, setGroupsUserOwns] = useState ([]);
  const [
    constructMiniSegments,
    setConstructMiniSegments,
  ] = useState([]); 
   const [
    constructMiniSegmentsPreFilter,
    setConstructMiniSegmentsPreFilter,
  ] = useState([]);
  ...
  console.log(
    "constructMiniSegmentContainer - right after initiation of formValues = ",
    formValues
  );

  console.log(
    "constructMiniSegmentContainer - right after start of currentCriteria = ",
    currentCriteria
  );
  console.log(
    "constructMiniSegmentContainer - right after start of currentAccess = ",
    currentAccess
  );
  ...
  {
    console.log(
      "constructMiniSegmentContainer - checkCurrentCriteria = ",
      currentCriteria
    );
    if (valuesToCheck != undefined && valuesToCheck != null)
    {
      ...
      {
        console.log(
          "constructMiniSegmentContainer - checkCurrentCriteria - Criteria found to have changed, valuesToCheck = ",
          valuesToCheck
        );
        criteriaDispatch({
        ...
        console.log(
          "constructMiniSegmentContainer - checkCurrentCriteria - Criteria did not change" );
      }
    }      else{
      console.log(
        "constructMiniSegmentContainer - checkCurrentCriteria - Criteria did not change" );
...
{
  console.log(
    "constructMiniSegmentContainer - checkCurrentAccess = ",
    currentAccess
  );
  if (valuesToCheck != undefined && valuesToCheck != null)
  {
    if (valuesToCheck.keepPrivate != currentAccess.keepPrivate ||
      ...
      {
        console.log(
          "constructMiniSegmentContainer - checkCurrentAccess - Access found to have changed, valuesToCheck = ",
          valuesToCheck
        );
      accessDispatch({
      ...
      console.log(
        "constructMiniSegmentContainer - checkCurrentAccess - Access did not change" );
    }
  }  else{
    console.log(
      "constructMiniSegmentContainer - checkCurrentAccess - Access did not change" );
  ...
  const markItemDeleted = (indexToDelete, markDeleted)=>{
    setConstructMiniSegments(
    [
      ...constructMiniSegments.slice(0, indexToDelete), // everything before current post
      {
        ...constructMiniSegments[indexToDelete],
        markDeleted: markDeleted,
      },
      ...constructMiniSegments.slice(indexToDelete + 1), // everything after current post
    ]);
    setConstructMiniSegmentsPreFilter(
      [
        ...constructMiniSegments.slice(0, indexToDelete), // everything before current post
  ...
  const updateConstructLineItem = (indexToUpdate, toggleDelete, lastIndexToUpdate, newCriteria, newAccess) => {
    console.log(
      "constructMiniSegmentContainer - updateConstructLineItem MiniSegment #indexToUpdate = ", indexToUpdate );
    const markDeleted = toggleDelete? (!constructMiniSegments[indexToUpdate].markDeleted):constructMiniSegments[indexToUpdate].markDeleted;
    console.log("constructMiniSegmentContainer - updateConstructLineItem constructMiniSegments[indexToUpdate] = ", constructMiniSegments[indexToUpdate] );
    let recordToUpdate = {};
    const id = constructMiniSegments[indexToUpdate]._id
    ...
    readById(
      ...
      if (!data) {
        console.log(
          "constructMiniSegment Container - updateConstructLineItem -readById - No Data Found!!!!"
        );
        recordToUpdate = null;
      } else {
        if (data.error) {
          console.log(
            "constructMiniSegment Container - updateConstructLineItem - Error!!!!", data.error
          );
          recordToUpdate = null;
        } else {
          console.log(
            "constructMiniSegment Container - updateConstructLineItem =  ",
            data
          );
          recordToUpdate = data;
          recordToUpdate.markDeleted = markDeleted
          updateConstructMiniSegment (recordToUpdate,indexToUpdate, lastIndexToUpdate, toggleDelete, newCriteria, newAccess)
        }
      }
    });    
  }

const updateConstructMiniSegment = (recordToUpdate, indexToUpdate, lastIndexToUpdate, toggleDelete, newCriteria, newAccess) =>
{
    ...
    update(
      ...
      if (!data) {
        console.log(
          "constructMiniSegmentContainer - updateConstructMiniSegment - No Data so dispatch, Check Criteria"
        );
      } else {
        if (data.error) {
          console.log(
            "constructMiniSegmentContainer - updateConstructMiniSegment - error returned = " + data.error
          );
        } else {
          console.log("constructMiniSegmentContainer -updateConstructMiniSegment -  Update Successful!!! data =  " + data); 
          if (toggleDelete)
          {
            console.log("constructMiniSegmentContainer - updates notComplete current index = ", indexToUpdate  )
            markItemDeleted(indexToUpdate,recordToUpdate.markDeleted)          
            setNotify({
              isOpen: true,
              message: "MiniSegment Toggle Deleted/Undeleted",
              type: "error",
            });
          }
 ...
 const prepareFormValues = (newFormValues) =>
  {
    console.log("ConstructMiniSegmentConatiner - preparingFormValues with newFormValues = ", newFormValues );
    setFormValues({
      ...
      subType: (newFormValues != null && newFormValues.subType)  ? newFormValues.subType : "MiniSegment",
      ...
      constructMiniSegment: (newFormValues != null && newFormValues.constructMiniSegment)  ? newFormValues.constructMiniSegment : "",
  ...
  useEffect(() => {
    ...
    console.log (" constructMiniSegmentContainer - useEffect - to populate the groupsUserOwns = ",userData.groupsUserOwns)
  let groupOptions = []  
  ...
  useEffect(() => {
    const abortController = new AbortController();
    const signal = abortController.signal;
    console.log("ConstructMiniSegmentContainer - inside useEffect()  ");
    getCurrentConstructByMaxUpdateDateAndUserId(
      ...
      if (!data) {
        console.log(
          "ConstructMiniSegmentContainer - No Data so dispatch, Check Criteria"
        );
      } else {
        if (data.error) {
          console.log(
            "ConstructMiniSegmentContainer - No Data so error is returned data =: ",
            data
          );
          console.log(
            "ConstructMiniSegmentContainer - No Data so error is returned data.error =: ",
            data.error
          );
          setRedirectToSignin(true);
        } else {
          console.log(
            "ConstructMiniSegmentContainer - Right after data is returned data =: ",
            data
          );
          criteriaDispatch({
            type: "SET_CRITERIAFROMCURRENTITEM",
  ...
  useEffect(() => {
    ...
    const signal = abortController.signal;
      console.log(
        "constructMiniSegmentContainer - Update Listing whenever the critiera changes, currentCriteria = ",
        currentCriteria
      );
      if (currentCriteria != undefined && currentCriteria != null )
      {
        if (
          ...
          listByCriteria(
            ...
            if (!data) {
              console.log(
                "ConstructMiniSegmentImport - No Data so dispatch, Check Criteria"
              );
              setConstructMiniSegments([]);
              setConstructMiniSegmentsPreFilter([]);
            } else {
              if (data.error) {
                console.log(
                  "constructMiniSegmentContainer - error returned = " + data.error
                );
                setConstructMiniSegments([]);
                setConstructMiniSegmentsPreFilter([]);
              } else if (data.length > 0) {
                console.log(
                  "constructMiniSegmentContainer - Update listing by ListByCriteria =  ",
                  data
                );
                const resultArray = data.map(miniSegment => ({  topic: miniSegment.topic,
                                                              description: miniSegment.description,
                                                              owner_id: miniSegment.owner_id,
                                                              group_id: miniSegment.group_id,
                                                              constructDetail: "Min Reading Segments",
                                                              markDeleted: miniSegment.markDeleted,
                                                              _id:miniSegment._id,}));

                setConstructMiniSegments(resultArray);
                setConstructMiniSegmentsPreFilter(resultArray);
      ...
      <CssBaseline />
          <Grid container spacing={0} justify="center" className={classes.container} >         
            <Grid item xs={9} >             
                ...
                constructLineItems = {constructMiniSegments}
                setConstructLineItems = {setConstructMiniSegments}
                constructLineItemsPreFilter ={constructMiniSegmentsPreFilter}
                setConstructLineItemsPreFilter ={setConstructMiniSegmentsPreFilter}
