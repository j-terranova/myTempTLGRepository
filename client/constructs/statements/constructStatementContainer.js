...
import { listByCriteria, update, readById } from "./api-constructStatement.js";
...
import { getCurrentConstructByMaxUpdateDateAndUserId } from "./api-constructStatement";
...
export default function ConstructStatementContainer() {
  ...
  const formTitle = "Statement Entry and Update Form";
  const listingTitle = "Statement Listing";
  const [listAllWithoutCriteria, setListAllWithoutCriteria] = useState(false);
  const [groupsUserOwns, setGroupsUserOwns] = useState ([]);
  const [
    constructStatements,
    setConstructStatements,
  ] = useState([]); 
   const [
    constructStatementsPreFilter,
    setConstructStatementsPreFilter,
  ] = useState([]);
  ...
  {
    console.log(
      "constructStatementContainer - checkCurrentCriteria = ",
      currentCriteria
    );
    if (valuesToCheck != undefined && valuesToCheck != null)
    {
      ...
      {
        console.log(
          "constructStatementContainer - checkCurrentCriteria - Criteria found to have changed, valuesToCheck = ",
          valuesToCheck
        );
        criteriaDispatch({
        ...
        console.log(
          "constructStatementContainer - checkCurrentCriteria - Criteria did not change" );
      }
    }      else{
      console.log(
        "constructStatementContainer - checkCurrentCriteria - Criteria did not change" );
...
{
  console.log(
    "constructStatementContainer - checkCurrentAccess = ",
    currentAccess
  );
  if (valuesToCheck != undefined && valuesToCheck != null)
  {
    if (valuesToCheck.keepPrivate != currentAccess.keepPrivate ||
      ...
      {
        console.log(
          "constructStatementContainer - checkCurrentAccess - Access found to have changed, valuesToCheck = ",
          valuesToCheck
        );
      accessDispatch({
      ...
      console.log(
        "constructStatementContainer - checkCurrentAccess - Access did not change" );
    }
  }  else{
    console.log(
      "constructStatementContainer - checkCurrentAccess - Access did not change" );
  ...
  const markItemDeleted = (indexToDelete, markDeleted)=>{
    setConstructStatements(
    [
      ...constructStatements.slice(0, indexToDelete), // everything before current post
      {
        ...constructStatements[indexToDelete],
        markDeleted: markDeleted,
      },
      ...constructStatements.slice(indexToDelete + 1), // everything after current post
    ]);
    setConstructStatementsPreFilter(
      [
        ...constructStatements.slice(0, indexToDelete), // everything before current post
  ...
  const updateConstructLineItem = (indexToUpdate, toggleDelete, lastIndexToUpdate, newCriteria, newAccess) => {
    console.log(
      "constructStatementContainer - updateConstructLineItem Statement #indexToUpdate = ", indexToUpdate );
    const markDeleted = toggleDelete? (!constructStatements[indexToUpdate].markDeleted):constructStatements[indexToUpdate].markDeleted;
    console.log("constructStatementContainer - updateConstructLineItem constructStatements[indexToUpdate] = ", constructStatements[indexToUpdate] );
    let recordToUpdate = {};
    const id = constructStatements[indexToUpdate]._id
    ...
    readById(
      ...
      if (!data) {
        console.log(
          "constructStatement Container - updateConstructLineItem -readById - No Data Found!!!!"
        );
        recordToUpdate = null;
      } else {
        if (data.error) {
          console.log(
            "constructStatement Container - updateConstructLineItem - Error!!!!", data.error
          );
          recordToUpdate = null;
        } else {
          console.log(
            "constructStatement Container - updateConstructLineItem =  ",
            data
          );
          recordToUpdate = data;
          recordToUpdate.markDeleted = markDeleted
          updateConstructStatement (recordToUpdate,indexToUpdate, lastIndexToUpdate, toggleDelete, newCriteria, newAccess)
        }
      }
    });    
  }

const updateConstructStatement = (recordToUpdate, indexToUpdate, lastIndexToUpdate, toggleDelete, newCriteria, newAccess) =>
{
    ...
    update(
      ...
      if (!data) {
        console.log(
          "constructStatementContainer - updateConstructStatement - No Data so dispatch, Check Criteria"
        );
      } else {
        if (data.error) {
          console.log(
            "constructStatementContainer - updateConstructStatement - error returned = " + data.error
          );
        } else {
          console.log("constructStatementContainer -updateConstructStatement -  Update Successful!!! data =  " + data); 
          if (toggleDelete)
          {
            console.log("constructStatementContainer - updates notComplete current index = ", indexToUpdate  )
            markItemDeleted(indexToUpdate,recordToUpdate.markDeleted)          
            setNotify({
              isOpen: true,
              message: "Statement Toggle Deleted/Undeleted",
              type: "error",
            });
          }
 ...
 const prepareFormValues = (newFormValues) =>
  {
    console.log("ConstructStatementConatiner - preparingFormValues with newFormValues = ", newFormValues );
    setFormValues({
      ...
      subType: (newFormValues != null && newFormValues.subType)  ? newFormValues.subType : "Statement",
      ...
      constructStatement: (newFormValues != null && newFormValues.constructStatement)  ? newFormValues.constructStatement : "",
  ...
  useEffect(() => {
    ...
    console.log (" constructStatementContainer - useEffect - to populate the groupsUserOwns = ",userData.groupsUserOwns)
  let groupOptions = []  
  ...
  useEffect(() => {
    const abortController = new AbortController();
    const signal = abortController.signal;
    console.log("ConstructStatementContainer - inside useEffect()  ");
    getCurrentConstructByMaxUpdateDateAndUserId(
      ...
      if (!data) {
        console.log(
          "ConstructStatementContainer - No Data so dispatch, Check Criteria"
        );
      } else {
        if (data.error) {
          console.log(
            "ConstructStatementContainer - No Data so error is returned data =: ",
            data
          );
          console.log(
            "ConstructStatementContainer - No Data so error is returned data.error =: ",
            data.error
          );
          setRedirectToSignin(true);
        } else {
          console.log(
            "ConstructStatementContainer - Right after data is returned data =: ",
            data
          );
          criteriaDispatch({
            type: "SET_CRITERIAFROMCURRENTITEM",
  ...
  useEffect(() => {
    ...
    const signal = abortController.signal;
      console.log(
        "constructStatementContainer - Update Listing whenever the critiera changes, currentCriteria = ",
        currentCriteria
      );
        ...
        if (
          ...
          listByCriteria(
            ...
            if (!data) {
              ...
              if (data.error) {
                console.log(
                  "constructStatementContainer - error returned = " + data.error
                );
                setConstructStatements([]);
                setConstructStatementsPreFilter([]);
              } else if (data.length > 0) {
                console.log(
                  "constructStatementContainer - Update listing by ListByCriteria =  ",
                  data
                );
                const resultArray = data.map(statement => ({  topic: statement.topic,
                                                              description: statement.description,
                                                              owner_id: statement.owner_id,
                                                              group_id: statement.group_id,
                                                              constructDetail: statement.constructStatement,
                                                              markDeleted: statement.markDeleted,
                                                              _id:statement._id,}));

                setConstructStatements(resultArray);
                setConstructStatementsPreFilter(resultArray);
      ...
      <CssBaseline />
          <Grid container spacing={0} justify="center" className={classes.container} >         
            <Grid item xs={9} >             
                ...
                constructLineItems = {constructStatements}
                setConstructLineItems = {setConstructStatements}
                constructLineItemsPreFilter ={constructStatementsPreFilter}
                setConstructLineItemsPreFilter ={setConstructStatementsPreFilter}
