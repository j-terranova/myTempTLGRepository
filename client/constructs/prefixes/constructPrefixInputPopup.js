...
import { create, update } from "./api-constructPrefix.js";
import ConstructPrefixForm from "./constructPrefixForm";
import ConstructPrefixEditor from "./constructPrefixEditor";
...
export default function ConstructPrefixInputPopup(props) {
  const { formValues,
    setFormValues,
    setOpenPopup,
    setSelected,
  ...
  setConstructLineItems(
    [
      ...constructLineItems,
      {
        ...
        constructDetail: newConstructLineItem.constructPrefix.prefix,
  ...
  setConstructLineItems(
    [
      ...constructLineItems.slice(0, index), // everything before current post
      {
        ...
        constructDetail: updatedConstructLineItem.constructPrefix.prefix,
...
const createNewConstructPrefix = () => {
  console.log("ConstructPrefixInputPopup - createNewConstructPrefix = ");
  const newDate = Date.now();
  console.log(
    "ConstructPrefixInputPopup - just before call to create prefix formValues = ",
    formValues
  );
  let newConstruct =  {
      ...
      constructPrefix: formValues.constructPrefix ,
  ...
  create(
    ...
    if (!data) {
      console.log(
        "ConstructPrefixInputPopup - No Data so dispatch, Check Criteria"
      );
      setNotify({
        isOpen: true,
        message: "Prefix not created! Please try again.. Notify Administrator if problem persists.",
        type: "error",
      });
    } else {
      if (data.error) {
        console.log(
          "ConstructPrefixInputPopup - error returned = " + data.error
        );
        setNotify({
          isOpen: true,
          message: "Prefix not created! Please try again.. Notify Administrator if problem persists.",
        ...
        console.log("ConstructPrefixInputPopup - after create - data = ", data );
        console.log("ConstructPrefixInputPopup - after create - data = ", data._id );
        ...
        console.log("ConstructPrefixInputPopup - after create - formValues = ", formValues );
        setNotify({
          isOpen: true,
...
const updateExistingConstructPrefix = (matchingIndex) => {
...
update(
    ...
    if (!data) {
      ...
      setNotify({
        isOpen: true,
        message: "Prefix not update! Please try again.. Notify Administrator if problem persists.",
        type: "error",
      });
    } else {
      if (data.error) {
        ...
        console.log("ConstructPrefixInputPopup - after create - preparingFormValues with data = ", data );
        console.log("ConstructPrefixInputPopup - after create - preparingFormValues with data = ", data._id );
...
const SaveEntries = () => {
  ...
  console.log(
    "ConstructPrefixInputPopup - SaveEntries clicked before create/update formValues = ",
    formValues
  );
  if (
    formValues.topic.length == 0 ||
    formValues.description.length == 0 ||
    formValues.constructPrefix.prefix.length == 0
  ) {
    setFormValues({
      ...formValues,
      modified: true,
      isError: true,
      errorMessage:
        "Error alert!!! You must have an entry for topic, description and prefix before saving!",
    });
    return;
  }
  if (
  ...
  console.log("ConstructPrefixInputPopup - search for id =  ", formValues._id); 
  const matchingIndex = findMatchingIndex(formValues._id);
  console.log(
    "ConstructPrefixInputPopup - after matchingIndex =  " + matchingIndex
  );
  if (matchingIndex >= 0) {
    updateExistingConstructPrefix(matchingIndex);
  } else {
    createNewConstructPrefix();
  }
  console.log("ConstructPrefixInputPopup End of Save Entries !!");
};
