...
import { loadGutenbergEquationData } from "./api-constructEquation.js";
...
export default function ConstructEquationImport(props) {
...
console.log("ConstructEquationImport" );
...
console.log("ConstructEquationImport - after setImportValues = ",importValues );
  ...
  const loadFile = () => {
    loadGutenbergEquationData(
      ...
      if (!data) {
        ...
        if (data.error) {
          console.log(
            "ConstructEquationImport - load error returned = " + data.error
          );
        } else {
          console.log("ConstructEquationImport - load data =  " + data);

          }
          setNotify({
            isOpen: true,
            message: "File Load of Equations Completed",
            type: "success",
          });
        }
      }
        ...
        <Form onSubmit={handleSubmit}>
            ...
            <Grid item xs={12}>
              <Typography variant="h6" className={classes.title}>
                Each new equation record will include entries as you have assigned them on this form.
                You can enter or leave blank entries for topic, tittle and/or description.
                Imput file must be comma delimited.  
                Input file must contain the equation.  The file may also contain; topic, title and description.
                ...
                Equation must always be included.  It can be the only file entry if other data is on the form.
              </Typography>
            </Grid>

            <Grid item xs={6}>
                   <Controls.Button
                    text="Import Equations from Equation File"
                    variant="contained"
                    color = "primary"
                    endIcon={<ImportExportIcon/>}
                    onClick={handleImportEquations}
                />
            </Grid> 
            <Grid item xs={6}>
                <Controls.Button
            ...
            <Grid item xs={6}>
                ...
                <Grid item xs={6}>
                    <div>
                        ...
                        Access to be applied to imported equations
                ...
                Criteria/Tags to be applied to imported equations
