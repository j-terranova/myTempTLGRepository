...
export default function ConstructEquationEditor(props) {
  const{  formValues, 
          setFormValues,
          addNewConstruct,
          SaveEntries,
...
const [equationTypeOptions, setEquationTypeOptions] = useState(
...
*/
  const [constructAssoc, setconstructAssoc] = useState(
    {
      wordToAssociate: formValues.constructEquation.wordToAssociate,
      equationType: formValues.constructEquation.equationType?formValues.constructEquation.equationType:"Antonym",
      associatedWords: formValues.constructEquation.associatedWords.length>0?formValues.constructEquation.associatedWords:[""],
    }
  );
  ...
  const validate = (fieldValues = constructAssoc) => {
    ...
    if ('equationType' in fieldValues)
        temp.equationType = fieldValues.equationType ? "" : "This field is required."
  ...
  const handleSave = (e) => {
    e.preventDefault()
    console.log("ConstructEquationEditor - handleSave - start");
    console.log("ConstructEquationEditor - handleSave - start - constructAssoc.associatedWords", constructAssoc.associatedWords);
    if (constructAssoc.associatedWords == undefined || constructAssoc.associatedWords == null)
    {
      console.log("ConstructEquationEditor - handleSave - Should show error that no associated word");
      setFormValues({
        ...formValues,
        isError: true,
        errorMessage:
...
const handleAddNew = () => {
  console.log("ConstructEquationEditor - handleAddNew - Start")
  console.log("ConstructEquationEditor - handleAddNew - formValues.modified = ", formValues.modified)
  if (formValues.modified)
  {
    setConfirmDialog({
      isOpen: true,
      ...
      onConfirm: () => {
        console.log("ConstructEquationEditor - right before validate")
        if (validate()) {
          console.log("ConstructEquationEditor - After validate right before save")
          addNewConstruct(); 
        }
      }
    }); 
  ...
  {
    console.log("ConstructEquationEditor - in else when modified not true")
    addNewConstruct();
  }
}
  ...
  const handleInputChange = (event) => {
    ...
    console.log("ConstructEquationEditor - handleInputChange - constructAssoc = ", constructAssoc);
    setFormValues({
      ...formValues,
      modified: true
    })
...
const handleAssociatedWordChange = (index, event) => {
  ...
  useEffect(() => {
    console.log(
      "ConstructEquationEditor - Inside handleClick UseEffect constructAssoc = ",
      constructAssoc)
      setFormValues({...formValues, constructEquation : constructAssoc, isError: false,
        errorMessage:""});
  }, [constructAssoc]);
        ...
        <Grid container alignItems="center">
            ...
            <Grid item xs={3}>
             <Controls.Select
                name="equationType"
                label="Type"
                value={constructAssoc.equationType}
                onChange={event => handleInputChange(event)}
                options={equationTypeOptions}
                error = {errors.equationType}
              />
            </Grid>
          </Grid>
          <Grid item xs={12}>
            <div>
              <Typography>
                Associated Words(s) - add one or more equations - list in order of preference
              </Typography>
            </div>
          </Grid>
