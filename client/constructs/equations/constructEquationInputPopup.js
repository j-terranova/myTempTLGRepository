...
import { create, update } from "./api-constructEquation.js";
import ConstructEquationForm from "./constructEquationForm";
import ConstructEquationEditor from "./constructEquationEditor";
...
export default function ConstructEquationInputPopup(props) {
  const { formValues,
    setFormValues,
    setOpenPopup,
    setSelected,
  ...
  setConstructLineItems(
    [
      ...constructLineItems,
      {
        ...
        constructDetail: newConstructLineItem.constructEquation.wordToAssociate,
  ...
  setConstructLineItems(
    [
      ...constructLineItems.slice(0, index), // everything before current post
      {
        ...
        constructDetail: updatedConstructLineItem.constructEquation.wordToAssociate,
...
const createNewConstructEquation = () => {
  console.log("ConstructEquationInputPopup - createNewConstructEquation = ");
  const newDate = Date.now();
  console.log(
    "ConstructEquationInputPopup - just before call to create equation formValues = ",
    formValues
  );
  let newConstruct =  {
      ...
      constructEquation: formValues.constructEquation ,
  ...
  create(
    ...
    if (!data) {
      console.log(
        "ConstructEquationInputPopup - No Data so dispatch, Check Criteria"
      );
      setNotify({
        isOpen: true,
        message: "Equation not created! Please try again.. Notify Administrator if problem persists.",
        type: "error",
      });
    } else {
      if (data.error) {
        console.log(
          "ConstructEquationInputPopup - error returned = " + data.error
        );
        setNotify({
          isOpen: true,
          message: "Equation not created! Please try again.. Notify Administrator if problem persists.",
        ...
        console.log("ConstructEquationInputPopup - after create - data = ", data );
        console.log("ConstructEquationInputPopup - after create - data = ", data._id );
        ...
        console.log("ConstructEquationInputPopup - after create - formValues = ", formValues );
        setNotify({
          isOpen: true,
...
const updateExistingConstructEquation = (matchingIndex) => {
...
update(
    ...
    if (!data) {
      ...
      setNotify({
        isOpen: true,
        message: "Equation not update! Please try again.. Notify Administrator if problem persists.",
        type: "error",
      });
    } else {
      if (data.error) {
        ...
        console.log("ConstructEquationInputPopup - after create - preparingFormValues with data = ", data );
        console.log("ConstructEquationInputPopup - after create - preparingFormValues with data = ", data._id );
...
const SaveEntries = () => {
  ...
  console.log(
    "ConstructEquationInputPopup - SaveEntries clicked before create/update formValues = ",
    formValues
  );
  if (
    formValues.topic.length == 0 ||
    formValues.description.length == 0 ||
    formValues.constructEquation.wordToAssociate.length == 0
  ) {
    setFormValues({
      ...formValues,
      modified: true,
      isError: true,
      errorMessage:
        "Error alert!!! You must have an entry for topic, description and equation before saving!",
    });
    return;
  }
  if (
  ...
  console.log("ConstructEquationInputPopup - search for id =  ", formValues._id); 
  const matchingIndex = findMatchingIndex(formValues._id);
  console.log(
    "ConstructEquationInputPopup - after matchingIndex =  " + matchingIndex
  );
  if (matchingIndex >= 0) {
    updateExistingConstructEquation(matchingIndex);
  } else {
    createNewConstructEquation();
  }
  console.log("ConstructEquationInputPopup End of Save Entries !!");
};
