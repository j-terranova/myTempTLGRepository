...
import { listByCriteria, update, readById } from "./api-constructDefinition.js";
import ConstructDefinitionImport from "./constructDefinitionImport"
...
import { getCurrentConstructByMaxUpdateDateAndUserId } from "./api-constructDefinition";
...
export default function ConstructDefinitionContainer() {
  ...
  const formTitle = "Definition Entry and Update Form";
  const listingTitle = "Definition Listing";
  ...
  const [
    ...
    setConstructDefinitions,
  ] = useState([]); 
   const [
    constructDefinitionsPreFilter,
    setConstructDefinitionsPreFilter,
  ] = useState([]);
  ...
  console.log(
    "constructDefinitionContainer - right after initiation of formValues = ",
    formValues
  );

  console.log(
    "constructDefinitionContainer - right after start of currentCriteria = ",
    currentCriteria
  );
  console.log(
    "constructDefinitionContainer - right after start of currentAccess = ",
    currentAccess
  );
  ...
  {
    console.log(
      "constructDefinitionContainer - checkCurrentCriteria = ",
      currentCriteria
    );
    if (valuesToCheck != undefined && valuesToCheck != null)
    {
      ...
      {
        console.log(
          "constructDefinitionContainer - checkCurrentCriteria - Criteria found to have changed, valuesToCheck = ",
          valuesToCheck
        );
        criteriaDispatch({
        ...
        console.log(
          "constructDefinitionContainer - checkCurrentCriteria - Criteria did not change" );
      }
    }      else{
      console.log(
        "constructDefinitionContainer - checkCurrentCriteria - Criteria did not change" );
...
{
  console.log(
    "constructDefinitionContainer - checkCurrentAccess = ",
    currentAccess
  );
  if (valuesToCheck != undefined && valuesToCheck != null)
  {
    if (valuesToCheck.keepPrivate != currentAccess.keepPrivate ||
      ...
      {
        console.log(
          "constructDefinitionContainer - checkCurrentAccess - Access found to have changed, valuesToCheck = ",
          valuesToCheck
        );
      accessDispatch({
      ...
      console.log(
        "constructDefinitionContainer - checkCurrentAccess - Access did not change" );
    }
  }  else{
    console.log(
      "constructDefinitionContainer - checkCurrentAccess - Access did not change" );
  ...
  const markItemDeleted = (indexToDelete, markDeleted)=>{
    setConstructDefinitions(
    [
      ...constructDefinitions.slice(0, indexToDelete), // everything before current post
      {
        ...constructDefinitions[indexToDelete],
        markDeleted: markDeleted,
      },
      ...constructDefinitions.slice(indexToDelete + 1), // everything after current post
    ]);
    setConstructDefinitionsPreFilter(
      [
        ...constructDefinitions.slice(0, indexToDelete), // everything before current post
  ...
  const updateConstructLineItem = (indexToUpdate, toggleDelete, lastIndexToUpdate, newCriteria, newAccess) => {
    console.log(
      "constructDefinitionContainer - updateConstructLineItem Definition #indexToUpdate = ", indexToUpdate );
    const markDeleted = toggleDelete? (!constructDefinitions[indexToUpdate].markDeleted):constructDefinitions[indexToUpdate].markDeleted;
    console.log("constructDefinitionContainer - updateConstructLineItem constructDefinitions[indexToUpdate] = ", constructDefinitions[indexToUpdate] );
    let recordToUpdate = {};
    const id = constructDefinitions[indexToUpdate]._id
    ...
    readById(
      ...
      if (!data) {
        console.log(
          "constructDefinition Container - updateConstructLineItem -readById - No Data Found!!!!"
        );
        recordToUpdate = null;
      } else {
        if (data.error) {
          console.log(
            "constructDefinition Container - updateConstructLineItem - Error!!!!", data.error
          );
          recordToUpdate = null;
        } else {
          console.log(
            "constructDefinition Container - updateConstructLineItem =  ",
            data
          );
          recordToUpdate = data;
          recordToUpdate.markDeleted = markDeleted
          updateConstructDefinition (recordToUpdate,indexToUpdate, lastIndexToUpdate, toggleDelete, newCriteria, newAccess)
        }
      }
    });    
  }

const updateConstructDefinition = (recordToUpdate, indexToUpdate, lastIndexToUpdate, toggleDelete, newCriteria, newAccess) =>
{
    ...
    update(
      ...
      if (!data) {
        console.log(
          "constructDefinitionContainer - updateConstructDefinition - No Data so dispatch, Check Criteria"
        );
        setNotify({
          isOpen: true,
          message: "Definition update failed! Please try again.. Notify Administrator if problem persists.",
          type: "error",
        });
      } else {
        if (data.error) {
          console.log(
            "constructDefinitionContainer - updateConstructDefinition - error returned = " + data.error
          );
          setNotify({
            isOpen: true,
            message: "Definition update failed! Please try again.. Notify Administrator if problem persists.",
          ...
          console.log("constructDefinitionContainer -updateConstructDefinition -  Update Successful!!! data =  " + data); 
          if (toggleDelete)
          {
            console.log("constructDefinitionContainer - updates notComplete current index = ", indexToUpdate  )
            markItemDeleted(indexToUpdate,recordToUpdate.markDeleted)          
            setNotify({
              isOpen: true,
              message: "Definition Toggle Deleted/Undeleted",
              type: "error",
            });
          }
          if (indexToUpdate == lastIndexToUpdate)
          {
            ...
            setNotify({
              isOpen: true,
              message: "Definition record(s) updated!",
              type: "success",
            });
          }
        }
 ...
 const prepareFormValues = (newFormValues) =>
  {
    console.log("ConstructDefinitionConatiner - preparingFormValues with newFormValues = ", newFormValues );
    setFormValues({
      ...
      subType: (newFormValues != null && newFormValues.subType)  ? newFormValues.subType : "Definition",
      ...
      constructDefinition: (newFormValues != null && newFormValues.constructDefinition)  ? newFormValues.constructDefinition : {
                wordToDefine: "",
                soundOut: "",
                partOfSpeech:  "",
                etymology: "",
  ...
  useEffect(() => {
    ...
    console.log (" constructDefinitionContainer - useEffect - to populate the groupsUserOwns = ",userData.groupsUserOwns)
  let groupOptions = []  
  ...
  useEffect(() => {
    const abortController = new AbortController();
    const signal = abortController.signal;
    console.log("ConstructDefinitionContainer - inside useEffect()  ");
    getCurrentConstructByMaxUpdateDateAndUserId(
      ...
      if (!data) {
        console.log(
          "ConstructDefinitionContainer - No Data so dispatch, Check Criteria"
        );
      } else {
        if (data.error) {
          console.log(
            "ConstructDefinitionContainer - No Data so error is returned data =: ",
            data
          );
          console.log(
            "ConstructDefinitionContainer - No Data so error is returned data.error =: ",
            data.error
          );
          setRedirectToSignin(true);
        } else {
          console.log(
            "ConstructDefinitionContainer - Right after data is returned data =: ",
            data
          );
          criteriaDispatch({
            type: "SET_CRITERIAFROMCURRENTITEM",
  ...
  useEffect(() => {
    ...
    const signal = abortController.signal;
      console.log(
        "constructDefinitionContainer - Update Listing whenever the critiera changes, currentCriteria = ",
        currentCriteria
      );
        ...
        if (
          ...
          listByCriteria(
            ...
            if (!data) {
              ...
              if (data.error) {
                console.log(
                  "constructDefinitionContainer - error returned = " + data.error
                );
                setConstructDefinitions([]);
                setConstructDefinitionsPreFilter([]);
              } else if (data.length > 0) {
                console.log(
                  "constructDefinitionContainer - Update listing by ListByCriteria =  ",
                  data
                );
                const resultArray = data.map(definition => ({  topic: definition.topic,
                                                              description: definition.description,
                                                              owner_id: definition.owner_id,
                                                              group_id: definition.group_id,
                                                              constructDetail: definition.description,
                                                              markDeleted: definition.markDeleted,
                                                              _id:definition._id,}));

                setConstructDefinitions(resultArray);
                setConstructDefinitionsPreFilter(resultArray);
  ...
  const handleRequestImport = () => {
    console.log ("constructDefinitionContainer - setting popup for Import")
  ...
  return (
      ...
      <CssBaseline />
          <Grid container spacing={0} justify="center" className={classes.container} >         
            <Grid item xs={9} >             
                ...
                constructLineItems = {constructDefinitions}
                setConstructLineItems = {setConstructDefinitions}
                constructLineItemsPreFilter ={constructDefinitionsPreFilter}
                setConstructLineItemsPreFilter ={setConstructDefinitionsPreFilter}
    ...
    <Popup
          title="Import Multiple Definitions from a file"
          openPopup={openImportPopup}
          setOpenPopup={setOpenImportPopup}
      >
          <ConstructDefinitionImport
