...
import { create, update } from "./api-frameworkLesson.js";
import FrameworkLessonForm from "./frameworkLessonForm";
...
export default function FrameworkLessonInputPopup(props) {
  const { formValues,
          setFormValues,
          prepareFormValues,
          setOpenPopup,
    ...
    setFrameworkLineItems(
      [
        ...frameworkLineItems,
        {
          ...
          frameworkSolutionMethod: newFrameworkLineItem.frameworkSolutionMethod,
    ...
    setFrameworkLineItems(
      [
        ...frameworkLineItems.slice(0, index), // everything before current post
        {
          ...
          frameworkSolutionMethod: updatedFrameworkLineItem.frameworkSolutionMethod,
  ...
  const cloneCurrentFrameworkLesson = () => {
    console.log("FrameworkLessonForm - cloneCurrentFrameworkLesson = ");
    console.log(
      "FrameworkLessonForm - cloneCurrentFrameworkLesson - just before call to clone lesson formValues = ",
      formValues
    );
    ...
    create(
      ...
      if (!data) {
        console.log(
          "FrameworkLessonForm - cloneCurrentFrameworkLesson - No Data so dispatch, Check Criteria"
        );
        setNotify({
          isOpen: true,
          message: "FrameworkLessonForm - cloneCurrentFrameworkLesson - Lesson not created! Please try again.. Notify Administrator if problem persists.",
          type: "error",
        });
      } else {
        if (data.error) {
          console.log(
            "FrameworkLessonForm - cloneCurrentFrameworkLesson - error returned = " + data.error
          );
          setNotify({
            isOpen: true,
            message: "FrameworkLessonForm - cloneCurrentFrameworkLesson - Lesson not created! Please try again.. Notify Administrator if problem persists.",
          ...
          addLineItems(data)
          setSelected([]);
          console.log("FrameworkLessonForm - cloneCurrentFrameworkLesson - after create - formValues = ", formValues );
          setNotify({
            isOpen: true,
  ...
  const createNewFrameworkLesson = () => {
    console.log("FrameworkLessonForm - createNewFrameworkLesson = ");
    const newDate = Date.now();
    console.log(
      "FrameworkLessonForm - just before call to create lesson formValues = ",
      formValues
    );
    let newFramework =  {
    ...
    create(
      ...
      if (!data) {
        console.log(
          "FrameworkLessonForm - No Data so dispatch, Check Criteria"
        );
        setNotify({
          isOpen: true,
          message: "Lesson not created! Please try again.. Notify Administrator if problem persists.",
          type: "error",
        });
      } else {
        if (data.error) {
          console.log(
            "FrameworkLessonForm - error returned = " + data.error
          );
          setNotify({
            isOpen: true,
            message: "Lesson not created! Please try again.. Notify Administrator if problem persists.",
          ...
          console.log("FrameworkLessonForm - after create - data = ", data );
          console.log("FrameworkLessonForm - after create - data._id = ", data._id );
          ...
          console.log("FrameworkLessonForm - after create - formValues = ", formValues );
          setNotify({
            isOpen: true,
  ...
  const updateExistingFrameworkLesson = (matchingIndex) => {
  ...
  update(
      ...
      if (!data) {
        ...
        setNotify({
          isOpen: true,
          message: "Lesson not update! Please try again.. Notify Administrator if problem persists.",
          type: "error",
        });
      } else {
        if (data.error) {
          ...
          console.log("FrameworkLessonForm - after create - preparingFormValues with data = ", data );
          console.log("FrameworkLessonForm - after create - preparingFormValues with data = ", data._id );
  ...
  const SaveEntries = () => {
    ...
    console.log(
      "FrameworkLessonForm - SaveEntries clicked before create/update formValues = ",
      formValues
    );
    if (
      ...
      setFormValues({
        ...
        isError: true,
        errorMessage:
          "Error alert!!! You must have an entry for topic, description and lesson before saving!",
      });
      return;
    }
    if (
    ...
    console.log("FrameworkLessonForm - search for id =  ", formValues._id); 
    const matchingIndex = findMatchingIndex(formValues._id);
    
    if (matchingIndex >= 0) {
      console.log(
        "FrameworkLessonForm - index match found! matchingIndex =  " + matchingIndex
      );
      updateExistingFrameworkLesson(matchingIndex);
    } else {
      console.log(
        "FrameworkLessonForm - index match NOT found! matchingIndex =  " + matchingIndex
      );
      createNewFrameworkLesson();
    }
    console.log("FrameworkLessonForm End of Save Entries !!");
  };
          ...
          <Grid item xs={12} >
            ...
            {/*<Paper className={classes.paper}>   */}       
                <FrameworkLessonForm 
                      formValues = {formValues}
                      setFormValues = {setFormValues}
                      cloneCurrentFrameworkLesson = {cloneCurrentFrameworkLesson}
                      validate={validate}
                      errors={errors}
