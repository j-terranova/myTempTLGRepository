...
import { listByCriteria, update, readById } from "./api-frameworkLesson.js";
...
import { getCurrentFrameworkByMaxUpdateDateAndUserId } from "./api-frameworkLesson";
...
import { useEducationPreferences } from "../../contexts/EducationPreferencesContext";
...
export default function FrameworkLessonContainer() {
  ...
  const formTitle = "Lesson Entry and Update Form";
  ...
  const [
   ...
   const [
    frameworkLessonsPreFilter,
    setFrameworkLessonsPreFilter,
  ] = useState([]);
  ...
  const [formValues, setFormValues] = useState({
    topic:  educationPreferences.topic,
    description:   "",
    myClass:  educationPreferences.myClass,
    category:  educationPreferences.category,
    subject:  educationPreferences.subject,
    type:  "Framework",
    subType:  "Lesson",
    difficultyLevel:  educationPreferences.difficultyLevel,
    ageRange:  educationPreferences.ageRange,
    image_id:  "",
    imageFileName:  "",
    imageBase64:  "",
    imageBinary:  "",
    ...
    imageStr: "",
    owner_id: userId,
    group_id:  educationPreferences.group_id,
    keepPrivate:  educationPreferences.keepPrivate,
  ...
  console.log(
    "frameworkLessonContainer - right after initiation of formValues = ",
    formValues
  );

  console.log(
    "frameworkLessonContainer - right after start of currentCriteria = ",
    currentCriteria
  );
  console.log(
    "frameworkLessonContainer - right after start of currentAccess = ",
    currentAccess
  );
  ...
  {
    console.log(
      "frameworkLessonContainer - checkCurrentCriteria = ",
      currentCriteria
    );
    if (valuesToCheck != undefined && valuesToCheck != null)
    {
      ...
      {
        console.log(
          "frameworkLessonContainer - checkCurrentCriteria - Criteria found to have changed, valuesToCheck = ",
          valuesToCheck
        );
        criteriaDispatch({
        ...
        console.log(
          "frameworkLessonContainer - checkCurrentCriteria - Criteria did not change" );
      }
    }      else{
      console.log(
        "frameworkLessonContainer - checkCurrentCriteria - Criteria did not change" );
...
{
  console.log(
    "frameworkLessonContainer - checkCurrentAccess = ",
    currentAccess
  );
  if (valuesToCheck != undefined && valuesToCheck != null)
  {
    if (valuesToCheck.keepPrivate != currentAccess.keepPrivate ||
      ...
      {
        console.log(
          "frameworkLessonContainer - checkCurrentAccess - Access found to have changed, valuesToCheck = ",
          valuesToCheck
        );
      accessDispatch({
      ...
      console.log(
        "frameworkLessonContainer - checkCurrentAccess - Access did not change" );
    }
  }  else{
    console.log(
      "frameworkLessonContainer - checkCurrentAccess - Access did not change" );
  ...
  const markItemDeleted = (indexToDelete, markDeleted)=>{
    setFrameworkLessons(
    [
      ...frameworkLessons.slice(0, indexToDelete), // everything before current post
      {
        ...frameworkLessons[indexToDelete],
        markDeleted: markDeleted,
      },
      ...frameworkLessons.slice(indexToDelete + 1), // everything after current post
    ]);
    setFrameworkLessonsPreFilter(
      [
        ...frameworkLessons.slice(0, indexToDelete), // everything before current post
  ...
  const updateFrameworkLineItem = (indexToUpdate, toggleDelete, lastIndexToUpdate, newCriteria, newAccess) => {
    console.log(
      "frameworkLessonContainer - updateFrameworkLineItem Lesson #indexToUpdate = ", indexToUpdate );
    const markDeleted = toggleDelete? (!frameworkLessons[indexToUpdate].markDeleted):frameworkLessons[indexToUpdate].markDeleted;
    console.log("frameworkLessonContainer - updateFrameworkLineItem frameworkLessons[indexToUpdate] = ", frameworkLessons[indexToUpdate] );
    let recordToUpdate = {};
    const id = frameworkLessons[indexToUpdate]._id
    ...
    readById(
      ...
      if (!data) {
        console.log(
          "frameworkLesson Container - updateFrameworkLineItem -readById - No Data Found!!!!"
        );
        recordToUpdate = null;
      } else {
        if (data.error) {
          console.log(
            "frameworkLesson Container - updateFrameworkLineItem - Error!!!!", data.error
          );
          recordToUpdate = null;
        } else {
          console.log(
            "frameworkLesson Container - updateFrameworkLineItem =  ",
            data
          );
          recordToUpdate = data;
          recordToUpdate.markDeleted = markDeleted
          updateFrameworkLesson (recordToUpdate,indexToUpdate, lastIndexToUpdate, toggleDelete, newCriteria, newAccess)
        }
      }
    });    
  }

const updateFrameworkLesson = (recordToUpdate, indexToUpdate, lastIndexToUpdate, toggleDelete, newCriteria, newAccess) =>
{
    ...
    update(
      ...
      if (!data) {
        console.log(
          "frameworkLessonContainer - updateFrameworkLesson - No Data so dispatch, Check Criteria"
        );
        setNotify({
          isOpen: true,
          message: "Lesson update failed! Please try again.. Notify Administrator if problem persists.",
          type: "error",
        });
      } else {
        if (data.error) {
          console.log(
            "frameworkLessonContainer - updateFrameworkLesson - error returned = " + data.error
          );
          setNotify({
            isOpen: true,
            message: "Lesson update failed! Please try again.. Notify Administrator if problem persists.",
          ...
          console.log("frameworkLessonContainer -updateFrameworkLesson -  Update Successful!!! data =  " + data); 
          if (toggleDelete)
          {
            console.log("frameworkLessonContainer - updates notComplete current index = ", indexToUpdate  )
            markItemDeleted(indexToUpdate,recordToUpdate.markDeleted)          
            setNotify({
              isOpen: true,
              message: "Lesson Toggle Deleted/Undeleted",
              type: "error",
            });
          }
          if (indexToUpdate == lastIndexToUpdate)
          {
            ...
            setNotify({
              isOpen: true,
              message: "Lesson record(s) updated!",
              type: "success",
            });
          }
        }
 ...
 const prepareFormValues = (newFormValues) =>
  {
    console.log("FrameworkLessonConatiner - preparingFormValues with newFormValues = ", newFormValues );
    setFormValues({
      topic: (newFormValues != null && newFormValues.topic)  ? 
              newFormValues.topic : currentCriteria.topic ? currentCriteria.topic : educationPreferences.topic ,
      ...
      myClass: (newFormValues != null && newFormValues.myClass)  ? 
              newFormValues.myClass : currentCriteria.myClass ? currentCriteria.myClass : educationPreferences.myClass,
      category: (newFormValues != null && newFormValues.category)  ? 
              newFormValues.category : currentCriteria.category ? currentCriteria.category : educationPreferences.category,
      subject: (newFormValues != null && newFormValues.subject)  ?  
              newFormValues.subject : currentCriteria.subject ?  currentCriteria.subject : educationPreferences.subject,
      type: (newFormValues != null && newFormValues.type)  ? newFormValues.type : "Framework",
      subType: (newFormValues != null && newFormValues.subType)  ? newFormValues.subType : "Lesson",
      difficultyLevel: (newFormValues != null && newFormValues.difficultyLevel)  ? 
              newFormValues.difficultyLevel : currentCriteria.difficultyLevel ? currentCriteria.difficultyLevel : educationPreferences.difficultyLevel,
      ageRange: (newFormValues != null && newFormValues.ageRange)  ? 
              newFormValues.ageRange : currentCriteria.ageRange ? currentCriteria.ageRange : educationPreferences.ageRange,
      ...
      group_id: (newFormValues != null && newFormValues.group_id)  ? 
                newFormValues.group_id : currentAccess.group_id ? currentAccess.group_id : educationPreferences.group_id,
      keepPrivate: (newFormValues != null && (newFormValues.keepPrivate != undefined && newFormValues.keepPrivate != null))  ? 
                newFormValues.keepPrivate : currentAccess.keepPrivate ? currentAccess.keepPrivate : educationPreferences.keepPrivate,
      approvedForPublicUse: (newFormValues != null && (newFormValues.approvedForPublicUse != undefined && newFormValues.approvedForPublicUse != null))  ? 
       ...
       frameworkLayoutFormat: (newFormValues != null && newFormValues.frameworkLayoutFormat)  ? newFormValues.frameworkLayoutFormat : "Carousel",
       ...
       frameworkSolutionMethod: (newFormValues != null && newFormValues.frameworkSolutionMethod)  ? newFormValues.frameworkSolutionMethod : "PostGrading",
       frameworkIncludeSpeech: (newFormValues != null && newFormValues.frameworkIncludeSpeech)  ? newFormValues.frameworkIncludeSpeech :  false,
       frameworkIncludeTimer: (newFormValues != null && newFormValues.frameworkIncludeTimer)  ? newFormValues.frameworkIncludeTimer :  false,
       frameworkIncludeScoring: (newFormValues != null && newFormValues.frameworkIncludeScoring)  ? newFormValues.frameworkIncludeScoring : false,
...
let canUpdate = false;
console.log (" frameworkLessonContainer - checkIfUserCanUpdate - frameworkOwwnerId = ", frameworkOwwnerId);
console.log (" frameworkLessonContainer - checkIfUserCanUpdate - frameworkGroupId = ", frameworkGroupId);
console.log (" frameworkLessonContainer - checkIfUserCanUpdate - userId = ", userId);
console.log (" frameworkLessonContainer - checkIfUserCanUpdate - userData.groupsUserMembership = ", userData.groupsUserMembership);
if (userId == frameworkOwwnerId)
{
  canUpdate = true;
  console.log (" frameworkLessonContainer - checkIfUserCanUpdate - MATCH! userId == frameworkOwwnerId  canUpdate = ", canUpdate);
  ...
  console.log (" frameworkLessonContainer - checkIfUserCanUpdate - returned Value  canUpdate = ", canUpdate);
  ...
  useEffect(() => {
    ...
    console.log (" frameworkLessonContainer - useEffect - to populate the groupsUserOwns = ",userData.groupsUserOwns)
  let groupOptions = []  
  ...
  useEffect(() => {
    const abortController = new AbortController();
    const signal = abortController.signal;
    console.log("FrameworkLessonContainer - inside useEffect()  ");
    getCurrentFrameworkByMaxUpdateDateAndUserId(
      ...
      if (!data) {
        console.log(
          "FrameworkLessonContainer - No Data so dispatch, Check Criteria"
        );
      } else {
        if (data.error) {
          console.log(
            "FrameworkLessonContainer - No Data so error is returned data =: ",
            data
          );
          console.log(
            "FrameworkLessonContainer - No Data so error is returned data.error =: ",
            data.error
          );
          setRedirectToSignin(true);
        } else {
          console.log(
            "FrameworkLessonContainer - Right after data is returned data =: ",
            data
          );
          criteriaDispatch({
            type: "SET_CRITERIAFROMCURRENTITEM",
  ...
  useEffect(() => {
    ...
    const signal = abortController.signal;
      console.log(
        "frameworkLessonContainer - Update Listing whenever the critiera changes, currentCriteria = ",
        currentCriteria
      );
        ...
        if (
          ...
          listByCriteria(
            ...
            if (!data) {
              ...
              if (data.error) {
                console.log(
                  "frameworkLessonContainer - error returned = " + data.error
                );
                setFrameworkLessons([]);
                setFrameworkLessonsPreFilter([]);
              } else if (data.length > 0) {
                console.log(
                  "frameworkLessonContainer - Update listing by ListByCriteria =  ",
                  data
                );
                const resultArray = data.map(lesson => ({     topic: lesson.topic,
                                                              description: lesson.description,
                                                              owner_id: lesson.owner_id,
                                                              group_id: lesson.group_id,
                                                              frameworkLayoutFormat: lesson.frameworkLayoutFormat,
                                                              frameworkResponseFormat: lesson.frameworkResponseFormat,
                                                              frameworkPresentationMethod: lesson.frameworkPresentationMethod,
                                                              frameworkSolutionMethod: lesson.frameworkSolutionMethod,
                                                              markDeleted: lesson.markDeleted,
                                                              _id:lesson._id,}));

                setFrameworkLessons(resultArray);
                setFrameworkLessonsPreFilter(resultArray);
      ...
      <CssBaseline />
          <Grid container spacing={0} justify="center" className={classes.container} >         
            <Grid item xs={9} >             
                ...
                frameworkLineItems = {frameworkLessons}
                setFrameworkLineItems = {setFrameworkLessons}
                frameworkLineItemsPreFilter ={frameworkLessonsPreFilter}
                setFrameworkLineItemsPreFilter ={setFrameworkLessonsPreFilter}
