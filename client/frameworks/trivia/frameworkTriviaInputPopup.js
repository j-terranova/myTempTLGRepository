...
import { create, update } from "./api-frameworkTrivia.js";
import FrameworkTriviaForm from "./frameworkTriviaForm";
...
export default function FrameworkTriviaInputPopup(props) {
  const { formValues,
          setFormValues,
          prepareFormValues,
          setOpenPopup,
  ...
  const cloneCurrentFrameworkTrivia = () => {
    console.log("FrameworkTriviaForm - cloneCurrentFrameworkTrivia = ");
    console.log(
      "FrameworkTriviaForm - cloneCurrentFrameworkTrivia - just before call to clone trivia formValues = ",
      formValues
    );
    ...
    create(
      ...
      if (!data) {
        console.log(
          "FrameworkTriviaForm - cloneCurrentFrameworkTrivia - No Data so dispatch, Check Criteria"
        );
        setNotify({
          isOpen: true,
          message: "FrameworkTriviaForm - cloneCurrentFrameworkTrivia - Trivia not created! Please try again.. Notify Administrator if problem persists.",
          type: "error",
        });
      } else {
        if (data.error) {
          console.log(
            "FrameworkTriviaForm - cloneCurrentFrameworkTrivia - error returned = " + data.error
          );
          setNotify({
            isOpen: true,
            message: "FrameworkTriviaForm - cloneCurrentFrameworkTrivia - Trivia not created! Please try again.. Notify Administrator if problem persists.",
          ...
          addLineItems(data)
          setSelected([]);
          console.log("FrameworkTriviaForm - cloneCurrentFrameworkTrivia - after create - formValues = ", formValues );
          setNotify({
            isOpen: true,
            message: "Trivia Cloned",
            type: "success",
          });
        }
      }
  ...
  const createNewFrameworkTrivia = () => {
    console.log("FrameworkTriviaForm - createNewFrameworkTrivia = ");
    const newDate = Date.now();
    console.log(
      "FrameworkTriviaForm - just before call to create trivia formValues = ",
      formValues
    );
    let newFramework =  {
    ...
    create(
      ...
      if (!data) {
        console.log(
          "FrameworkTriviaForm - No Data so dispatch, Check Criteria"
        );
        setNotify({
          isOpen: true,
          message: "Trivia not created! Please try again.. Notify Administrator if problem persists.",
          type: "error",
        });
      } else {
        if (data.error) {
          console.log(
            "FrameworkTriviaForm - error returned = " + data.error
          );
          setNotify({
            isOpen: true,
            message: "Trivia not created! Please try again.. Notify Administrator if problem persists.",
          ...
          console.log("FrameworkTriviaForm - after create - data = ", data );
          console.log("FrameworkTriviaForm - after create - data._id = ", data._id );
          ...
          console.log("FrameworkTriviaForm - after create - formValues = ", formValues );
          setNotify({
            isOpen: true,
  ...
  const updateExistingFrameworkTrivia = (matchingIndex) => {
  ...
  update(
      ...
      if (!data) {
        ...
        setNotify({
          isOpen: true,
          message: "Trivia not update! Please try again.. Notify Administrator if problem persists.",
          type: "error",
        });
      } else {
        if (data.error) {
          ...
          console.log("FrameworkTriviaForm - after create - preparingFormValues with data = ", data );
          console.log("FrameworkTriviaForm - after create - preparingFormValues with data = ", data._id );
  ...
  const SaveEntries = () => {
    ...
    console.log(
      "FrameworkTriviaForm - SaveEntries clicked before create/update formValues = ",
      formValues
    );
    if (
      ...
      setFormValues({
        ...
        isError: true,
        errorMessage:
          "Error alert!!! You must have an entry for topic, description and trivia before saving!",
      });
      return;
    }
    if (
    ...
    console.log("FrameworkTriviaForm - search for id =  ", formValues._id); 
    const matchingIndex = findMatchingIndex(formValues._id);
    
    if (matchingIndex >= 0) {
      console.log(
        "FrameworkTriviaForm - index match found! matchingIndex =  " + matchingIndex
      );
      updateExistingFrameworkTrivia(matchingIndex);
    } else {
      console.log(
        "FrameworkTriviaForm - index match NOT found! matchingIndex =  " + matchingIndex
      );
      createNewFrameworkTrivia();
    }
    console.log("FrameworkTriviaForm End of Save Entries !!");
  };
          ...
          <Grid item xs={12} >
            ...
            {/*<Paper className={classes.paper}>   */}       
                <FrameworkTriviaForm 
                      formValues = {formValues}
                      setFormValues = {setFormValues}
                      cloneCurrentFrameworkTrivia = {cloneCurrentFrameworkTrivia}
                      validate={validate}
                      errors={errors}
